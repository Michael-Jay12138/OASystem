(function (app) {
    var ListController = function ($scope, userService) {
        ...
        function getPageCount() {
            $.get("../user/GetDataCount").then(function (result) {
        ...
        var getByPage = function (pageIndex) {
            if (pageIndex < 1 || pageIndex > $scope.cn)
                return;
            $.get("../user/GetUserListByPage?" + "pageIndex=" + pageIndex + "&pageSize=" + $scope.pageSize).then(function (result) {
                var users = JSON.parse(result);
                $scope.$root.users = users;
                $scope.$apply();
